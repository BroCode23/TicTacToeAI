...
void Tuple::set_score(int score) {
	this->score = score;
	return;
}
...
int play(string player_1_name, string player_2_name) {
	...
	if (player_1_name == "human" || player_2_name == "human") {
...
void make_move(int board[3][3], Tuple move_coords, int player){ //places an 1 or 0 in the spot for move coords for each player
	if (player == 1) {
		board[move_coords.get_y()][move_coords.get_x()] = 1;
	}
	else {
		board[move_coords.get_y()][move_coords.get_x()] = 2;
	}
	return;
}
...
bool is_valid_move(int board[3][3], Tuple coord) {
	if (coord.get_x() >= 0 && coord.get_x() <= 2) { //must be within x range
		if (coord.get_y() >= 0 && coord.get_y() <= 2) { //within y range
			if (board[coord.get_y()][coord.get_x()] == 0) { //must be an open spot
				return true;
			}
		}
	}
...
Tuple random_ai(int board[3][3], int player) { //player is 1 or 2
	...
	for (int y = 0; y < 3; y++) {
		for (int x = 0; x < 3; x++) {
			if (board[y][x] == 0) {
				possible_moves[number_of_moves].set_y(y);
				possible_moves[number_of_moves].set_x(x);
				number_of_moves++;
			}
		}
	}
...
Tuple winning_moves_ai(int board[3][3], int player) {
	...
	for (int x = 0; x < 3; x++) {
		if ((board[0][x] == player && board[1][x] == player) || (board[0][x] == player && board[2][x] == player) || (board[1][x] == player && board[2][x] == player)) { //checks all columns
			if (board[0][x] == 0) {
				winning_move.set_y(0);
				winning_move.set_x(x);
				return winning_move;
			}
			else if (board[1][x] == 0){
				winning_move.set_y(1);
				winning_move.set_x(x);
				return winning_move;
			}
			else if (board[2][x] == 0) {
				winning_move.set_y(2);
				winning_move.set_x(x);
				return winning_move;
			}
		}
	...
	for (int y = 0; y < 3; y++) {
		if ((board[y][0] == player && board[y][1] == player) || (board[y][0] == player && board[y][2] == player) || (board[y][1] == player && board[y][2] == player)) { //checks all rows
			if (board[y][0] == 0) {
				winning_move.set_y(y);
				winning_move.set_x(0);
				return winning_move;
			}
			else if (board[y][1] == 0) {
				winning_move.set_y(y);
				winning_move.set_x(1);
				return winning_move;
			}
			else if (board[y][2] == 0) {
				winning_move.set_y(y);
				winning_move.set_x(2);
				return winning_move;
			}
		}
	}
...
int minimax_score(int board[3][3], int player) {
	...
	for (int y = 0; y < 3; y++) { //checks for a tie
		for (int x = 0; x < 3; x++) {
			if (board[y][x] == 0) { //if there is any open spot on the board
				...
				possible_moves[number_of_moves].set_y(y); //assigns the spot as a possible move
				possible_moves[number_of_moves].set_x(x);
				number_of_moves++;
			}
		}
	...
	for (int i = 0; i < number_of_moves; i++) { //iterate through all the possible moves
		...
		if (player == 1) {
			possible_moves[i].set_score(minimax_score(new_board, 2)); //changes players and the process repeats
			if (possible_moves[i].get_score() == 10) { // checks early if the score is the best
				return 10;
			}
		}
		else {
			possible_moves[i].set_score(minimax_score(new_board, 1));
			if (possible_moves[i].get_score() == -10) { // checks early
				return -10;
			}
		}
	}
	...
	for (int i = 0; i < number_of_moves; i++) {
		if (i == 0) { //ensures a return value
			best_score = possible_moves[i].get_score();
		}
		if (player == 1) { //returns highest value if player 1
			if (possible_moves[i].get_score() > best_score) {
				best_score = possible_moves[i].get_score();
			}
		}
	...
	for (int y = 0; y < 3; y++) { // iterates through all possible moves
		for (int x = 0; x < 3; x++) {
			if (board[y][x] == 0) { //fill array of possible moves
				...
				if (player == 1) { 
					possible_moves[number_of_moves].set_score(minimax_score(new_board, 2));
					if (possible_moves[number_of_moves].get_score() == 10) {
						return possible_moves[number_of_moves];
					}
				}
				else {
					possible_moves[number_of_moves].set_score(minimax_score(new_board, 1));
					if (possible_moves[number_of_moves].get_score() == -10) {
						return possible_moves[number_of_moves];
					}
				}
